...
class FeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        self.dwconv1 = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1, groups=hidden_features, bias=bias, dilation=1)
        self.dwconv2 = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=3, groups=hidden_features, bias=bias, dilation=3)
        self.dwconv3 = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=5, groups=hidden_features, bias=bias, dilation=5)
    ...
    def forward(self, x,k_v):
        ...
        x2_1 = self.dwconv1(x2)
        x2_2 = self.dwconv1(x2)
        x2_3 = self.dwconv1(x2)
        x2 = x2_1 + x2_2 + x2_3
        x = x1 * x2
        x = self.project_out(x)
        return x
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.qkv = nn.Conv2d(dim, dim*3, kernel_size=1, bias=bias)
        self.bypass = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.qkv_dwconv1 = nn.Conv2d(dim*3, dim*3, kernel_size=3, stride=1, padding=1, groups=dim*3, bias=bias, dilation=1)
        self.qkv_dwconv2 = nn.Conv2d(dim*3, dim*3, kernel_size=3, stride=1, padding=3, groups=dim*3, bias=bias, dilation=3)
        self.qkv_dwconv3 = nn.Conv2d(dim*3, dim*3, kernel_size=3, stride=1, padding=5, groups=dim*3, bias=bias, dilation=5)
        self.project_out = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
    ...
    def forward(self, x, k_v):
        ...
        qkv = self.qkv(x)
        byconv = self.bypass(x)
        qkv1 = self.qkv_dwconv1(qkv)
...
class CPEN(nn.Module):
    def __init__(self,n_feats = 64, n_encoder_res = 6):
        ...
        E1=[nn.Conv2d(48, n_feats, kernel_size=3, padding=1),
            nn.LeakyReLU(0.1, True)]
        E2 = [
            *[common.ResBlock(common.default_conv, n_feats, kernel_size=3) for _ in range(n_encoder_res)],
            MambaLayer(dim=n_feats)
        ]
